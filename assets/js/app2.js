...
//		storedLimits: localStorage.getItem('limits'),
		//	Methods
		...
		changeChannel: function() {
			...
			console.log('storedChannels.length is ' + this.storedChannels.length);
...
console.log('prev input val: ' + $('.input').val());
				$('.input').val('');
console.log('new input val: ' + $('.input').val());
		...
		changeGif: function() {
			...
			if(tv.channels.num != 0) {
				...
				if(tv.channels.num == 1) {
					...
					ajaxURL = 'https://api.giphy.com/v1/gifs/search?lang=en&api_key=9Q3EhEbfCX4PIZY6SSPqQnSSngkJBDM7'
							+ '&limit=' + tv.channels.storedLimits[tv.channels.num] + '&q=' + tv.channels.storedChannels[tv.channels.num];
							console.log('API CH: ' + tv.channels.num + '(' + tv.channels.storedChannels[tv.channels.num]
							+ '); API Limit: ' + tv.channels.storedLimits[tv.channels.num]);
							console.log('storedLimits: ' + tv.channels.storedLimits);
				}
				//	perform the ajax call
				$.ajax({
					url: ajaxURL,
...
/*
					console.log('new state: ' + tv.gifs.state);
					console.log('new still: ' + tv.gifs.still);
					console.log('new animate: ' + tv.gifs.animate);
					console.log('new name: ' + tv.gifs.name);
					console.log('new rating: ' + tv.gifs.rating);
*/
...
*/
			...
			//	if on the favorites channel and nothing has been favorited, display a default blank screen
				...
				console.log('should be blank screen');
...
console.log('#chan-' + this.scrollId);
			},
...
console.log('channels length: ' + tv.channels.storedChannels.length);
console.log('channels length % 4: ' + tv.channels.storedChannels.length % 4);	
					});
...
console.log('Scroll Id: ' + this.scrollId);
console.log('selected: ' + this.selected);
				}
				//	toggles each display
/*
				$('.guide-display').toggleClass('js-hidden');
				$('.pause').toggleClass('js-hidden');
				$('.ok').toggleClass('js-hidden');
...
//				tv.menus.guide.scrollId = tv.menus.guide.mod4Floor(tv.menus.guide.selected);
				if(tv.menus.guide.selected % 4 == 0) {
					tv.menus.guide.scrollId = tv.menus.guide.scrollId - 4;
				} else {
//					tv.menus.guide.scrollId = tv.menus.guide.mod4Floor(tv.menus.guide.selected);
				}
				tv.menus.guide.scrollId < 0 ? tv.menus.guide.scrollId = tv.channels.storedChannels.length - 1 : tv.menus.guide.scrollId = 0;
	...
	numberPad: {
		...
		processNums: function() {
			...
			if(parseInt(this.numVal) < tv.channels.storedChannels.length) {
				//	sets the channel number to the inptted string (converted to an integer)
...
$(document).ready( function() {
	console.log('stored limits: ' + tv.channels.storedLimits);
